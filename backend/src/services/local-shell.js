...
function normalizeProjectEnv(value) {
  ...
  for (const [key, rawValue] of entries) {
    ...
    if (key === "CLAUDE_CONFIG_DIR" && rawValue !== CLAUDE_CONFIG_DIR) {
      throw new TypeError(`CLAUDE_CONFIG_DIR is reserved and must be ${CLAUDE_CONFIG_DIR}`);
    }
...
function mergeShellEnvironment(globalEnv, projectEnv) {
  const merged = normalizeProjectEnv(Object.assign(
    Object.create(null),
    globalEnv,
    projectEnv
  ));
  merged.CLAUDE_CONFIG_DIR = CLAUDE_CONFIG_DIR;
  return merged;
}
...
async function ensureShellRecoveryFiles(project, name, shellConfig = {}) {
  ...
  const claudeModelList = CLAUDE_CHAT_MODELS
    .map(({ id, context, compact }) => `${id}\t${context}\t${compact}`)
    .join("\\n");
  const claudeImageList = CLAUDE_IMAGE_MODELS.join("\\n");
  const claudeLargestContext = CLAUDE_LARGEST_CONTEXT;
  const rc = [
    ...
    "}",
    "__reaper_resume_config=",
    "__reaper_prepare_claude_resume() {",
    "  __reaper_resume_config=",
    "  local __resume_id= __arg __take=0 __store __d __p __cwd",
    "  for __arg in \"$@\"; do",
    "    if [ \"$__take\" = 1 ]; then __resume_id=$__arg; break; fi",
    "    case \"$__arg\" in --resume) __take=1 ;; --resume=*) __resume_id=${__arg#--resume=}; break ;; esac",
    "  done",
    "  [ -n \"$__resume_id\" ] || return 0",
    "  for __store in \"${CLAUDE_CONFIG_DIR:-/work/.reaper/claude}\" /root/.claude; do",
    "    [ -d \"$__store\" ] || continue",
    "    for __d in \"$__store\"/projects/*/\"$__resume_id\" \"$__store\"/projects/*/\"$__resume_id\".jsonl; do",
    "      [ -e \"$__d\" ] || continue",
    "      __reaper_resume_config=$__store",
    "      __p=${__d#\"$__store\"/projects/}",
    "      __p=${__p%/\"$__resume_id\"}",
    "      __p=${__p%/\"$__resume_id\".jsonl}",
    "      __p=${__p#-}",
    "      __cwd=/${__p//-//}",
    "      if [ -d \"$__cwd\" ]; then cd -- \"$__cwd\" || true; fi",
    "      return 0",
    "    done",
    "  done",
    "}",
    "__reaper_claude() {",
    "  local __model=${REAPER_CLAUDE_MODEL-} __context=1000000 __compact=800000 __arg __take=0",
    "  for __arg in \"$@\"; do",
    "    if [ \"$__take\" = 1 ]; then __model=$__arg; break; fi",
    "    case \"$__arg\" in --model) __take=1 ;; --model=*) __model=${__arg#--model=}; break ;; esac",
    "  done",
    "  case \"$__model\" in",
    claudeChatCases,
    "  esac",
    "  CLAUDE_CONFIG_DIR=${__reaper_resume_config:-${CLAUDE_CONFIG_DIR:-/work/.reaper/claude}} IS_SANDBOX=1 CLAUDE_CODE_MAX_CONTEXT_TOKENS=${CLAUDE_CODE_MAX_CONTEXT_TOKENS:-$__context} CLAUDE_CODE_AUTO_COMPACT_WINDOW=${CLAUDE_CODE_AUTO_COMPACT_WINDOW:-$__compact} command claude \"$@\"",
    "}",
    "reaper-claude-model() {",
    ...
    "  case \"$__model\" in",
    claudeChatCases,
    `    ${claudeImageCase}) printf 'Reaper: %s is image-only and cannot be used for Claude chat.\\n' "$__model" >&2; return 2 ;;`,
    "    *) printf 'Reaper: unknown Claude chat model: %s. Run reaper-claude-models to list IDs.\\n' \"$__model\" >&2; return 2 ;;",
    "  esac",
    "  for __arg in \"$@\"; do case \"$__arg\" in --continue|--resume|--resume=*) __resume=1 ;; esac; done",
    "  if [ \"$__resume\" = 1 ]; then",
    "    __current_context=",
    "    case \"${REAPER_CLAUDE_MODEL-}\" in",
    claudeChatCases.replaceAll("__context=", "__current_context=").replaceAll("; __compact=", "; : __compact="),
    "    esac",
    `    [ -n "$__current_context" ] || __current_context=${claudeLargestContext}`,
    "    if [ \"$__context\" -lt \"$__current_context\" ]; then",
    "      printf 'Reaper: refusing unsafe resume from %s-token context to %s (%s tokens). Claude Code 2.1.212 cannot be proven to compact and resume noninteractively before switching.\\n' \"$__current_context\" \"$__model\" \"$__context\" >&2",
    `      printf 'Manually precompact first: reaper-claude-model %s --continue; run /compact in Claude; exit Claude; then run reaper-claude-model %s --continue (or pass the same --resume arguments).\\n' "\${REAPER_CLAUDE_MODEL:-a ${claudeLargestContext}-token model}" "$__model" >&2`,
    "      return 3",
    "    fi",
    "  fi",
    ...
    "  __reaper_prepare_claude_resume \"$@\"",
    "  CLAUDE_CODE_MAX_CONTEXT_TOKENS=$__context CLAUDE_CODE_AUTO_COMPACT_WINDOW=$__compact __reaper_claude --model \"${__claude_id:-$__model}\" \"$@\"",
    `  local __status=$?; rm -f ${interrupted}; return "$__status"`,
    "}",
    `claude() { __reaper_mark claude; __reaper_prepare_claude_resume "$@"; __reaper_claude "$@"; local __status=$?; rm -f ${interrupted}; return "$__status"; }`,
    "reaper-resume() {",
    ...
    `  local __status; case "$__cmd" in opencode) command opencode --continue ;; omp) command omp --continue ;; claude) if [ -n "\${REAPER_CLAUDE_MODEL-}" ]; then reaper-claude-model "$REAPER_CLAUDE_MODEL" --continue; else printf 'Reaper: interrupted Claude session has no model metadata. Resume it manually with reaper-claude-model <id> --continue.\\n' >&2; return 3; fi ;; *) printf 'Reaper: cannot resume %s automatically.\\n' "$__cmd" >&2; return 2 ;; esac; __status=$?; rm -f ${interrupted}; return "$__status"`,
    "}",
