...
async function ensureShellRecoveryFiles(project, name, shellConfig = {}) {
  ...
  const rc = [
    ...
    "}",
    "__reaper_claude() { local __claude_bin=claude; [ ! -x /usr/local/bin/claude-real ] || __claude_bin=/usr/local/bin/claude-real; IS_SANDBOX=1 \"$__claude_bin\" --dangerously-skip-permissions \"$@\"; }",
    "reaper-claude-model() {",
    ...
    "  esac",
    "  for __arg in \"$@\"; do case \"$__arg\" in --continue|--resume) __resume=1 ;; esac; done",
    "  if [ \"$__resume\" = 1 ]; then",
    "    __current_context=",
