...
export const FLAGS = Object.freeze({ FINAL: 0x01, ERROR: 0x02 });
...
const CLIENT_TO_SERVER_TYPES = new Set([TYPES.HELLO, TYPES.OPEN, TYPES.INPUT, TYPES.RESIZE]);
...
export function encodePing(timestamp) {
  let value;
  if (typeof timestamp === "bigint") value = timestamp;
  else if (Number.isSafeInteger(timestamp) && timestamp >= 0) value = BigInt(timestamp);
  else throw new RangeError("PING timestamp must be a non-negative safe integer or bigint");
